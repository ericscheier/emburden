...
version_path <- "NatureComm_figure-latex"#"net_energy_equity_final_review20211217200747_files"

knitr::opts_chunk$set(echo=FALSE)
```

# What Kind of ROI are People Getting from the Energy System?

```{r, fig.cap=""}
knitr::include_graphics(paste(version_path,"continental-map-1.pdf",sep="/"))
```
...
```{r, fig.cap=""}
knitr::include_graphics(paste(version_path,"inset-map-1.pdf",sep="/"))
```
...
knitr::include_graphics(paste(version_path,"density-charts-1.pdf",sep="/"))
```
...
```{r states, fig.cap=""}
knitr::include_graphics(paste(version_path,"state-violin-1.pdf",sep="/"))
```
...
+ 16% of households (1 in 6) experience energy poverty as presently defined (>6% of household income on energy expenditures)
+ 94% of households below the federal poverty line also face energy poverty
+ More than 5.2m of those above the federal poverty line (1 in 6) face this scarcity
...
+ Expanding the burden scope to other costs (transportation, housing, food, goods, & services)
+ Extending the underlying methodology over time scale
+ Ground-truthing estimates with better consumption and income data
+ Exploring the potential of distributed energy resources to alleviate energy burdens
+ Assessing vulnerability more holistically (natural disasters, historical disadvantages)
+ Incorporating better understanding of human behavior (hidden energy burden)
+ Informing program and rate design for state, utility, and industry stakeholders

# Implications for Service Providers

+ DER adoption potentials & trends
+ Residential program design
+ Retail rate design
+ Wholesale market design

```{r cca, fig.cap="", out.height="50%"}
knitr::include_graphics("scp_poster_map.pdf")
```
...
+ ~200k customers
+ 500 MW peak net demand
+ 200MW of self-consumption
+ High climate risk
+ Rural, wealthy, culturally diverse
}

# Prosumer Potential

```{r prosumer, fig.cap="",out.height="80%"}
knitr::include_graphics("state_prosumer_ratios.png")
```

# Market Driver: Electrification

```{r electrify, fig.cap="", out.height="80%"}
knitr::include_graphics("state_prosumer_ratios_electrify.png")
```

# What Kind of ROI are Americans Getting from the Energy System?
...
\]
\[E_{B}^{*} = \frac{S}{G} = 6\%\]
\[N_{h}^{*} \approx 16 \Rightarrow Household\ at\ Energy\ Poverty\ Line\]
...
```{r, fig.cap="", out.height = '50%', include=FALSE}
knitr::include_graphics(paste(version_path,"metric-comparison-1.pdf",sep="/"))
```

```{r load-table-full, include=FALSE}
print_table <- read_csv(paste(version_path,"comparison_table.csv",sep="/"))
# print_table <- read_csv("comparison_table.csv")
```

```{r table-full, results="asis", include=TRUE}
